...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/19 22:22:27 by gpassos-          #+#    #+#             */
/*   Updated: 2021/03/19 22:22:28 by gpassos-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

t_cn	get_hex_pnumber_t(t_cn input, int *counter)
{
	t_cn conv;
...
t_cn	get_hex_pnumber(t_cn input, int *counter)
{
	...
	else
	{
			...
			(conv.saida.flags.pad_zeros == 0 || conv.saida.flags.minus == 1))
		{
			...
			conv = get_hex_pnumber_t(conv, counter);
	}
	return (conv);
}
