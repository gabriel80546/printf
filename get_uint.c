...
t_cn get_uint_init(t_cn input)
{
	...
	conv = input;
	conv.tt = 0;
	if (conv.saida.flags.n_right == 0 && conv.saida.flags.n_left == 0 && conv.saida.flags.right_asteristic == 1 && conv.n_ui == 0)
		conv.tt = 1;
	return (conv);
}
...
t_cn	get_uint_pnumber(t_cn input, int *counter)
{
	...
	{
		if (conv.n_ui == 0 && conv.saida.flags.precision == 1 && (conv.saida.flags.pad_zeros == 0 || conv.saida.flags.minus == 1))
		{
		...
		{
			if (conv.n_ui == 0 && conv.saida.flags.n_left_indf == 0 && conv.saida.flags.n_right_indf == 0 && conv.saida.flags.n_left != 0 && conv.saida.flags.n_right == 0)
			{
				conv.temp = ft_calloc(1, 1);
				ft_pchar(' ', counter);
			}
